...
module.exports = function(grunt) {
	...
	grunt
			.initConfig({
				...
				// Automatically inject Bower components into the app and
				// karma.conf.js
				wiredep : {
					options : {
				...
				concurrent : {
					server : [ 'newer:babel:client', 'sass', ],
					test : [ 'newer:babel:client', 'sass', ],
					debug : {
					...
					dist : [ 'newer:babel:client', 'sass', 'imagemin' ]
				},
				...
				mochaTest : {
					options : {
						...
						timeout : 5000
					// set default mocha spec timeout
					},
					unit : {
						src : [ 'server/**/*.spec.js' ]
					},
	...
	grunt.registerTask('serve', function(target) {
		...
		if (target === 'debug') {
			return grunt.task.run([ 'clean:server', 'env:all', 'injector:sass',
					'concurrent:server', 'injector', 'wiredep:client',
					'postcss', 'concurrent:debug' ]);
		}

		grunt.task.run([ 'clean:server', 'env:all', 'injector:sass',
				'concurrent:server', 'injector', 'wiredep:client', 'postcss',
	...
	grunt.registerTask('test', function(target, option) {
		if (target === 'server') {
			return grunt.task.run([ 'env:all', 'env:test', 'mochaTest:unit',
					'mochaTest:integration' ]);
		}

		else if (target === 'client') {
			return grunt.task.run([ 'clean:server', 'env:all', 'injector:sass',
					'concurrent:test', 'injector', 'postcss', 'wiredep:test',
					'karma' ]);
		}
			...
			if (option === 'prod') {
				return grunt.task.run([ 'build', 'env:all', 'env:prod',
						'express:prod', 'protractor' ]);
			}

			else {
				return grunt.task.run([ 'clean:server', 'env:all', 'env:test','db:clean',
						'injector:sass', 'concurrent:test', 'injector',
						'wiredep:client', 'postcss', 'express:dev',
						'protractor' ]);
			}
		}
			...
			if (option === 'unit') {
				...
				return grunt.task.run([ 'env:all', 'env:test',
						'mocha_istanbul', 'istanbul_check_coverage' ]);
			}
	...
	grunt.registerTask('build', [ 'clean:dist', 'injector:sass',
			'concurrent:dist', 'injector', 'wiredep:client', 'useminPrepare',
			'postcss', 'ngtemplates', 'concat', 'ngAnnotate', 'copy:dist',
			'cdnify', 'cssmin', 'uglify', 'filerev', 'usemin' ]);
